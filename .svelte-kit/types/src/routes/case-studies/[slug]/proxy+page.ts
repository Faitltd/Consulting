...
const caseStudyDetails: Record<string, CaseStudyDetail> = {
  'construction-remodeler-efficiency': {
    ...
    highlights: ['Scope templates', 'Approval trails', 'Readiness dashboard']
  },
  'field-services-visibility': {
    ...
    highlights: ['Live status board', 'Alerting rules', 'Customer updates']
  },
  'professional-services-playbook': {
    ...
    highlights: ['Scope templates', 'Delivery playbook', 'Utilization dashboard']
  }
};

export const load = ({ params }: Parameters<PageLoad>[0]) => {
  const base = caseStudies.find((cs) => cs.slug === params.slug);
